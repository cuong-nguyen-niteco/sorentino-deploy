...
angular.module('sorentinoApp', ['sorentinoApp.constants', 'ngCookies', 'ngResource', 'ngSanitize', 'ui.router', 'jkuri.gallery']).config(["$urlRouterProvider", "$locationProvider", "$stateProvider", function ($urlRouterProvider, $locationProvider, $stateProvider) {
...
(function () {
  var CreateProductController = function () {
    ...
    function CreateProductController($http, $stateParams, $state) {
      ...
      CKEDITOR.replace('specDesc');
      CKEDITOR.replace('specDetail');
      CKEDITOR.replace('specMat');
      ...
      if (this.product.spec) {
        CKEDITOR.instances.specDesc.setData(this.product.spec.description);
        CKEDITOR.instances.specDetail.setData(this.product.spec.details);
        CKEDITOR.instances.specMat.setData(this.product.spec.material);
      }
    ...
    _createClass(CreateProductController, [{
      ...
      value: function buildSpec() {
        return {
          description: CKEDITOR.instances.specDesc.getData(),
          details: CKEDITOR.instances.specDetail.getData(),
          material: CKEDITOR.instances.specMat.getData()
        };
      }
    }, {
      key: 'submitProduct',
